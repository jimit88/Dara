...
(function(window, document, location, $, undefined) {
    "use strict"; /*var main_img_width=$('div.gallery-placeholder').width();*/
    var main_img_width = document.getElementsByClassName("gallery-placeholder")[0].clientWidth;
    ...
    function findShadowEdge(pos, min, max, dir) {
        return min === max ? !1 : dir === 'vertical' ? (pos <= min ? 'top' : pos >= max ? 'bottom' : 'top bottom') : (pos <= min ? 'left' : pos >= max ? 'right' : 'left right')
    }
    ...
    var UTIL = (function() {
        ...
        function isExpectedCaption(frameItem, isExpected, undefined) {
            var expected = !1,
                frameExpected;
            frameItem.showCaption === undefined || frameItem.showCaption === !0 ? frameExpected = !0 : frameExpected = !1;
            if (!isExpected) {
                return !1
            }
    ...
    function touch($el, options) {
        var el = $el[0],
            tail = {},
            touchEnabledFLAG, startEvent, $target, controlTouch, touchFLAG, targetIsSelectFLAG, targetIsLinkFlag, tolerance, moved;
    ...
    jQuery.Fotorama = function($fotorama, opts) {
            ...
            eachIndex(indexes, type, function(i, index, dataFrame, $frame, key, frameData) {
                ...
                function error() {
                    ...
                    if ((!dataFrame.html || type !== 'stage') && dummy && dummy !== src) {
                        ...
                        frameData.$full = null;
                        loadImg([index], type, specialMeasures, !0)
                    } else {
        ...
        function thumbsDraw(pos, loadFLAG) {
            ...
            $navThumbFrame.each(function() {
                ...
                if (thisData.l + thisData.w < leftLimit || exceedLimit || callFit(thisData.$img, specialMeasures)) return;
        ...
        function arrsUpdate() {
            ...
            $arrPrev.toggleClass(arrDisabledClass, disablePrev).attr(disableAttr(disablePrev, !1));
            $arrNext.toggleClass(arrDisabledClass, disableNext).attr(disableAttr(disableNext, !1))
        }
        ...
        function thumbArrUpdate() {
            ...
            if (opts.navtype === 'thumbs' && !opts.loop) {
                (activeIndex == 0) ? isLeftDisable = !0: isLeftDisable = !1;
                (activeIndex == opts.data.length - 1) ? isRightDisable = !0: isRightDisable = !1
            }
            if (opts.navtype === 'slides') {
                var pos = readPosition($navShaft, opts.navdir);
                pos >= navShaftTouchTail.max ? isLeftDisable = !0 : isLeftDisable = !1;
                pos <= navShaftTouchTail.min ? isRightDisable = !0 : isRightDisable = !1
            }
            $thumbArrLeft.toggleClass(arrDisabledClass, isLeftDisable).attr(disableAttr(isLeftDisable, !0));
            $thumbArrRight.toggleClass(arrDisabledClass, isRightDisable).attr(disableAttr(isRightDisable, !0))
        }
        ...
        function navUpdate() {
            ...
            toDeactivate[navFrameKey].push(activeFrame[navFrameKey].addClass(activeClass).attr('data-active', !0))
        }

        function deactivateFrames(key) {
            var _toDeactivate = toDeactivate[key];
            while (_toDeactivate.length) {
                _toDeactivate.shift().removeClass(activeClass).attr('data-active', !1)
            }
        }
        ...
        function stageShaftReposition(skipOnEnd) {
            ...
            if ($frame) {
                ...
                toDeactivate[STAGE_FRAME_KEY].push($frame.addClass(activeClass).attr('data-active', !0));
                if ($frame.hasClass(stageFrameClass)) {
                    $frame.attr('aria-hidden', 'false')
                }
                skipOnEnd || that.showStage.onEnd(!0);
                stop($stageShaft, 0, !0);
        ...
        that.showStage = function(silent, options, time) {
            ...
            var onEnd = that.showStage.onEnd = function(skipReposition) {
                ...
                changeAutoplay();
                if (that.fullScreen) {
                    activeFrame[STAGE_FRAME_KEY].find('.' + imgFullClass).attr('aria-hidden', !1);
                    activeFrame[STAGE_FRAME_KEY].find('.' + imgClass).attr('aria-hidden', !0)
                } else {
                    activeFrame[STAGE_FRAME_KEY].find('.' + imgFullClass).attr('aria-hidden', !0);
                    activeFrame[STAGE_FRAME_KEY].find('.' + imgClass).attr('aria-hidden', !1)
                }
            };
            if (!o_fade) {
                slide($stageShaft, {
        ...
        that.show = function(options) {
            that.longPress.singlePressInProgress = !0;
            var index = calcActiveIndex(options);
            calcGlobalIndexes(index);
            ...
            showedFLAG = typeof lastActiveIndex !== 'undefined' && lastActiveIndex !== activeIndex;
            lastActiveIndex = activeIndex;
            that.longPress.singlePressInProgress = !1;
            return this
        };
        ...
        that.resize = function(options) {
            ...
            if (measureIsValid(width)) {
                ...
                if (jQuery("body").hasClass("fotorama__fullscreen")) {
                    width = main_img_width
                } else if (jQuery("body").hasClass("weltpixel-quickview-catalog-product-view")) {
                    //width = 732
