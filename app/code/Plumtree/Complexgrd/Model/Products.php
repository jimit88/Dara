...
class Products extends \Magento\Framework\Model\AbstractModel implements IdentityInterface
{
    ...
    {
        $this->_init('Plumtree\Complexgrd\Model\ResourceModel\Products');
    }
    ...
    public function getProducts(\Plumtree\Complexgrd\Model\Products $object)
    {
		//echo '<pre>';
		$banner_data = $object->getData('banner');
		if($banner_data){
		if(isset($banner_data['banner_id'])){
        $tbl = 'jh_mageplaza_product_attachment_rel'; //$this->getResource()->getTable(\Plumtree\Complexgrd\Model\ResourceModel\Products::TBL_ATT_PRODUCT);
