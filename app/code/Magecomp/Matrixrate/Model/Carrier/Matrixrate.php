    ...
    \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    public function collectRates(RateRequest $request)
    {
       ...
       if (!$this->getConfigFlag('active')) {
		...
		foreach ($request->getAllItems() as $item) {
			$prod_uct = $productObject->loadByAttribute('sku', explode("-", $item->getSku())[0]);
			if($prod_uct){
				if($prod_uct->getAttributeText('freight_ship_only')=="Yes"){
					$matrix_rate = 1;
				}
			}
		}
		...
		\Magento\Framework\App\ObjectManager::getInstance()->get('Psr\Log\LoggerInterface')->debug(print_r(['set Condition Name' => $request->getConditionMRName()], true));
		...
		\Magento\Framework\App\ObjectManager::getInstance()->get('Psr\Log\LoggerInterface')->debug(print_r(['Rate Array' => $rateArray], true));
	...
	public function getCode($type, $code='')
    {
        ...
        if (!isset($codes[$type])) {
            throw new LocalizedException__('Invalid Matrix Rate code type: %1', $type);
        }
        ...
        if (!isset($codes[$type][$code])) {
            throw new LocalizedException__('Invalid Matrix Rate code for type %1: %2', $type, $code);
        }
