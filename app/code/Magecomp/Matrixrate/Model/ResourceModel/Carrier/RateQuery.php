...
class RateQuery
{
     ...
     */
    public function prepareSelect(\Magento\Framework\DB\Select $select,$zipRangeSet)
    {
        $select->where(
            'website_id = :website_id'
        )->order(
            ['dest_country_id DESC', 'dest_zip DESC', 'condition_from_value DESC']
        );//->limit(1);
        ...
        $orWhere = '(' . implode(
            ') OR (',
            [
                "dest_country_id = :country_id AND  dest_zip = :postcode",
				"dest_country_id = :country_id AND dest_zip = ''",
				/*==================*/
				"dest_country_id = :country_id AND $query = 0".$zipSearchString,
				"dest_country_id = :country_id AND dest_city = ''".$zipSearchString,
				"dest_country_id = :country_id AND dest_zip = '' AND $query = 0",
				"dest_country_id = :country_id AND $query = 0 ".$zipSearchString,
				"dest_country_id = :country_id AND $query = 0 AND dest_zip = ''",
				"dest_country_id = :country_id AND dest_city=''".$zipSearchString,
				"dest_country_id = :country_id AND dest_city='' AND dest_zip=''",
				"dest_country_id = :country_id AND dest_city='' AND dest_zip=''",
				"dest_country_id = '0' AND  dest_zip=''",
				"dest_country_id = '0' ".$zipSearchString,
				"dest_country_id = :country_id ".$zipSearchString,
    ...
    public function getBindings()
    {
		...
		\Magento\Framework\App\ObjectManager::getInstance()->get('Psr\Log\LoggerInterface')->debug(print_r(['Get Bindings' => $bind], true));
